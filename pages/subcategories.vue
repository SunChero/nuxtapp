<template>
  ...
  <v-card>
    <v-card-title>
      <v-btn color="default"  class="mb-2" @click="DB_SAVE_SUBCATEGORIES">Save SubCategories  </v-btn>
      <v-spacer></v-spacer>
    </v-card-title>
    ...
    <v-progress-linear slot="progress" color="blue" indeterminate></v-progress-linear>
      <template slot="items" slot-scope="props">
        <tr @click="props.expanded = !props.expanded ">
          ...
          <td class="text-xs-right">{{ props.item.TargetUrl }}</td>
...
import {mapState , mapMutations , mapActions} from 'vuex'
  export default {
    ...
    created () {
      this.$store.dispatch('WWW_ASK_INFO')
    },
    ...
    methods: {
      ...
      ...mapActions(['DB_SAVE_SUBCATEGORIES', 'WWW_ASK_INFO'])
    }
  }
</script>
