<template>
  ...
  <v-card>
    <v-card-title>
      <v-btn color="primary"  class="mb-2" @click="DB_SAVE_NEWS">Save NEWS</v-btn>
      <v-spacer></v-spacer>
      <v-text-field
        append-icon="search"
        label="Search"
    ...
    <v-progress-linear slot="progress" color="blue" indeterminate></v-progress-linear>
      <template slot="items" slot-scope="props">
        <tr @click="props.expanded = !props.expanded ">
          <td><a :href="url(props.item.PageUrl)">{{ props.item.Title }}</a></td>
          <td class="text-xs-right">{{ props.item.PageUrl }}</td>
          <td class="text-xs-right">{{ props.item.ImageUrl }}</td>
          <td class="text-xs-right">{{ props.item.ArticleDate }}</td>
      ...
      <template slot="expand" slot-scope="props">
        <v-card flat>
          <v-card-text>{{props.item.Excerpt}}</v-card-text>
        </v-card>
      </template>
  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'PageUrl', value: 'PageUrl' },
        { text: 'ImageUrl', value: 'ImageUrl' },
        { text: 'ArticleDate', value: 'ArticleDate' },
    ...
    created () {
      this.$store.dispatch('WWW_ASK_NEWS')
    },
    ...
    methods: {
      ...
      ...mapActions(['DB_SAVE_NEWS', 'WWW_ASK_NEWS'])
    }
  }
</script>
