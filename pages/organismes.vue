<template>
  ...
  <v-card>
    <v-card-title>
      <v-btn color="primary"  class="mb-2" @click="DB_SAVE_ORGS">Save Orgs</v-btn>
      <v-spacer></v-spacer>
      <v-text-field
        append-icon="search"
        label="Search"
    ...
    <v-progress-linear slot="progress" color="blue" indeterminate></v-progress-linear>
      <template slot="items" slot-scope="props">
        <tr @click="props.expanded = !props.expanded ">
          <td><a :href="url(props.item.url)">{{ props.item.Id }}</a></td>
          <td class="text-xs-right">{{ props.item.title }}</td>
          <td class="text-xs-right">{{ props.item.phone }}</td>
          <td class="text-xs-right">{{ props.item.addr }}</td>
          <td class="text-xs-right">{{ props.item.email }}</td>
          <td class="text-xs-right">{{ props.item.type}}</td>
          <td class="text-xs-right">{{ props.item.seteur }}</td>
...
import {mapState , mapMutations , mapActions} from 'vuex'
  export default {
    ...
    created () {
      this.$store.dispatch('WWW_ASK_ORGS')
    },
    ...
    methods: {
      ...
      ...mapActions(['DB_SAVE_ORGS', 'WWW_ASK_ORGS'])
    }
  }
</script>
